...
// assuming s1 is biger than s2
		console.log(s1results.result, oldResultPointer)
...
const poll = () => {
    getNewItems("foreststream", "landstream", (data) => {
        ...
        for(let i =0; i < data.length; i++) {
            getKeyItem("foreststream", data[i].key, (keyItem) => {
                ...
                let dataToSend = {
                    land_address: userData.land_address,
        ...
        //console.log(survey)
                if(survey[dataToSend.land_address] === undefined){
                        dataToSend.status = "false"
                        dataToSend.location = "nil"
                        sendToStream(requestId, dataToSend, "landstream");
                }else{
                    if(dataToSend.ownername == survey[dataToSend.land_address]['owner']){
                        dataToSend.status = "true"
                        dataToSend.place = survey[dataToSend.land_address].place
                    }else{
                        dataToSend.status = "false"
                        dataToSend.location = "nil"
                    }
...
function sendToStream(requestId,data){
    ...
    body: 
    { method: 'publish',
        params: [ 'landstream', requestId,hex ],
        id: 1,
        chain_name: 'tree' },
    json: true };
