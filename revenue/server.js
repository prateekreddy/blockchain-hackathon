...
const poll = () => {
    getNewItems("foreststream", "revenuestream", (data) => {
        ...
        //console.log(survey)
                if(survey[dataToSend.khata_no === undefined]){
                        dataToSend.status = "false"
                        dataToSend.location = "nil"
                        sendToStream(requestId, dataToSend, "revenuestream");
                }else{
                    if(dataToSend.ownername == survey[dataToSend.khata_no]['owner']){
                        dataToSend.status = "true"
                        dataToSend.place = survey[dataToSend.khata_no].place
                    }else{
                        dataToSend.status = "false"
                        dataToSend.location = "nil"
                    }
...
function sendToStream(requestId,data){
    ...
    body: 
    { method: 'publish',
        params: [ 'revenuestream', requestId,hex ],
        id: 1,
        chain_name: 'tree' },
    json: true };
